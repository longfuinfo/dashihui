 ...
 */
public class AdapterFrgServerDetailGoods extends AdapterBase<Map<String, Object>> {


    private List<Map<String, Object>> listObject;

    public AdapterFrgServerDetailGoods(Activity context, List<Map<String, Object>> _mList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.frag_server_detail_item0, null);
    ...
    static class ViewHolder {
        ...
        public void refresh(final int position) {
            if (!UtilList.isEmpty(mListMap)){
                mGoodsName.setText(mListMap.get(position).get("SERVICETITLE") + "");
                mPrice.setText("￥" + mListMap.get(position).get("AMOUNT"));

                if (mListMap.get(position).get("COUNT") != null && UtilString.isEmpty(mListMap.get(position).get("COUNT") + "")) {
