...
@SuppressLint("NewApi")
public class WgtDragLayout extends ViewGroup {
    ...
    public WgtDragLayout(Context context) {
        this(context, null);
    }

    public WgtDragLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public WgtDragLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void animTopOrBottom(View releasedChild, float yvel) {
        ...
        if (releasedChild == frameView1) {
            ...
            if (yvel < -VEL_THRESHOLD || (downTop1 == 0 && frameView1.getTop() < -DISTANCE_THRESHOLD)) {
                ...
                finalTop = -viewHeight;
                mPosition = 1;
                FragmentShopDetailPager.newInstance(getContext(), mPosition);
