package com.dashihui.afford.util.list;



import com.dashihui.afford.util.array.UtilArray;
...
public class UtilList {
	...
	/**
	 * 两个list做对比是否相等 compare two list
	 * 
	 * <pre>
	...
	public static <V> boolean isEquals(ArrayList<V> actual, ArrayList<V> expected) {
		...
		for (int i = 0; i < actual.size(); i++) {
			if (!UtilObject.isEquals(actual.get(i), expected.get(i))) {
				return false;
			}
		}
		return true;
	...
	public static <V> V getLast(List<V> sourceList, V value) {
		return (sourceList == null) ? null : (V) UtilArray.getLast(sourceList.toArray(), value, true);
	}
	...
	public static <V> V getNext(List<V> sourceList, V value) {
		return (sourceList == null) ? null : (V) UtilArray.getNext(sourceList.toArray(), value, true);
	}
